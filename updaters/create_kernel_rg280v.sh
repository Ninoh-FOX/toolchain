...
if [ -r "update_rg280v/mininit-syspart" ]; then
MININIT="update_rg280v/mininit-syspart"
else
MININIT=""
fi

if [ -r "update_rg280v/ubiboot-rg350.bin" ]; then
BOOTLOADERS="update_rg280v/ubiboot-rg350.bin"
else
BOOTLOADERS=""
fi
...
if [ "$BOOTLOADERS" ] ; then
	...
	echo -n "Calculating SHA1 sum of bootloaders... "
        sha1sum "$BOOTLOADERS" | cut -d' ' -f1 > "update_rg280v/ubiboot-rg350.bin.sha1"
        echo "done"

        BOOTLOADERS="$BOOTLOADERS update_rg280v/ubiboot-rg350.bin.sha1"
fi

if [ "$MININIT" ] ; then

	echo -n "Calculating SHA1 sum of mininit-syspart... "
	sha1sum "$MININIT" | cut -d' ' -f1 > "update_rg280v/mininit-syspart.sha1"
	echo "done"

	MININIT="$MININIT update_rg280v/mininit-syspart.sha1"
fi

echo "$DATE" > update_rg280v/date.txt

echo "$VERSION" > update_rg280v/version.txt
...
# Create OPK.
OPK_FILE=update_rg280v/rg280v-Kernel-update-$VERSION-$DATE.opk
mksquashfs \
	update_rg280v/default.gcw0.desktop \
	update_rg280v/rogue.png \
	update_rg280v/update.sh \
	update_rg280v/trimfat.py \
	update_rg280v/flash_partition.sh \
	update_rg280v/date.txt \
	update_rg280v/version.txt \
	update_rg280v/fsck.fat \
	$BOOTLOADERS \
	$MININIT \
	$KERNEL \
	$MODULES_FS \
...
rm update_rg280v/default.gcw0.desktop update_rg280v/date.txt update_rg280v/version.txt update_rg280v/*.sha1
