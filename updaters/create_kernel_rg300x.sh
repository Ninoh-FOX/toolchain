...
if [ -r "update_rg300x/ubiboot-rg350.bin" ]; then
BOOTLOADERS="update_rg300x/ubiboot-rg350.bin"
else
BOOTLOADERS=""
fi
...
if [ "$BOOTLOADERS" ] ; then
	...
	echo -n "Calculating SHA1 sum of bootloaders... "
        sha1sum "$BOOTLOADERS" | cut -d' ' -f1 > "update_rg300x/ubiboot-rg350.bin.sha1"
        echo "done"

        BOOTLOADERS="$BOOTLOADERS update_rg300x/ubiboot-rg350.bin.sha1"
fi
...
# Create OPK.
OPK_FILE=update_rg300x/RG300X-Kernel-update-$VERSION-$DATE.opk
mksquashfs \
