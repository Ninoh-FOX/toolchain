...
if [ -r "update_rg280m/ubiboot-rg350.bin" ]; then
BOOTLOADERS="update_rg280m/ubiboot-rg350.bin"
else
BOOTLOADERS=""
fi
...
if [ "$BOOTLOADERS" ] ; then
	...
	echo -n "Calculating SHA1 sum of bootloaders... "
        sha1sum "$BOOTLOADERS" | cut -d' ' -f1 > "update_rg280m/ubiboot-rg350.bin.sha1"
        echo "done"

        BOOTLOADERS="$BOOTLOADERS update_rg280m/ubiboot-rg350.bin.sha1"
fi
...
# Create OPK.
OPK_FILE=update_rg280m/rg280m-update-$VERSION-$DATE.opk
mksquashfs \
