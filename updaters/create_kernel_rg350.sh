...
if [ -r "update_rg350/mininit-syspart" ]; then
MININIT="update_rg350/mininit-syspart"
else
MININIT=""
fi

if [ -r "update_rg350/ubiboot-rg350.bin" ]; then
BOOTLOADERS="update_rg350/ubiboot-rg350.bin"
else
BOOTLOADERS=""
fi
...
if [ "$BOOTLOADERS" ] ; then
	...
	echo -n "Calculating SHA1 sum of bootloaders... "
        sha1sum "$BOOTLOADERS" | cut -d' ' -f1 > "update_rg350/ubiboot-rg350.bin.sha1"
        echo "done"

        BOOTLOADERS="$BOOTLOADERS update_rg350/ubiboot-rg350.bin.sha1"
fi

if [ "$MININIT" ] ; then

	echo -n "Calculating SHA1 sum of mininit-syspart... "
	sha1sum "$MININIT" | cut -d' ' -f1 > "update_rg350/mininit-syspart.sha1"
	echo "done"

	MININIT="$MININIT update_rg350/mininit-syspart.sha1"
fi

echo "$DATE" > update_rg350/date.txt

echo "$VERSION" > update_rg350/version.txt
...
# Write metadata.
cat > update_rg350/default.gcw0.desktop <<EOF
[Desktop Entry]
...
# Create OPK.
OPK_FILE=update_rg350/RG350-Kernel-update-$VERSION-$DATE.opk
mksquashfs \
	update_rg350/default.gcw0.desktop \
	update_rg350/rogue.png \
	update_rg350/update.sh \
	update_rg350/trimfat.py \
	update_rg350/flash_partition.sh \
	update_rg350/date.txt \
    update_rg350/version.txt \
    update_rg350/fsck.fat \
	$BOOTLOADERS \
	$MININIT \
	$KERNEL \
	$MODULES_FS \
...
rm update_rg350/default.gcw0.desktop update_rg350/date.txt update_rg350/version.txt update_rg350/*.sha1
