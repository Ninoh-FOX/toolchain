...
if [ -r "update_rg350m/mininit-syspart" ]; then
MININIT="update_rg350m/mininit-syspart"
else
MININIT=""
fi
...
if [ "$MININIT" ] ; then
	...
	sha1sum "$MININIT" | cut -d' ' -f1 > "update_rg350m/mininit-syspart.sha1"
	echo "done"

	MININIT="$MININIT update_rg350m/mininit-syspart.sha1"
fi

echo "$DATE" > update_rg350m/date.txt

echo "$VERSION" > update_rg350m/version.txt
...
# Write metadata.
cat > update_rg350m/default.gcw0.desktop <<EOF
[Desktop Entry]
...
# Create OPK.
OPK_FILE=update_rg350m/RG350M-Kernel-update-$VERSION-$DATE.opk
mksquashfs \
	update_rg350m/default.gcw0.desktop \
	update_rg350m/rogue.png \
	update_rg350m/update.sh \
	update_rg350m/trimfat.py \
	update_rg350m/flash_partition.sh \
	update_rg350m/date.txt \
	update_rg350m/version.txt \
	update_rg350m/fsck.fat \
	$BOOTLOADERS \
	$MININIT \
	$KERNEL \
	$MODULES_FS \
...
rm update_rg350m/default.gcw0.desktop update_rg350m/date.txt update_rg350m/version.txt update_rg350m/*.sha1
