...
PYTHON_PYGAME_OPT_DEPENDS += sdl_image sdl2_image
endif

ifeq ($(BR2_PACKAGE_PYTHON_PYGAME_FONT),y)
PYTHON_PYGAME_OPT_DEPENDS += sdl_ttf sdl2_ttf
endif

ifeq ($(BR2_PACKAGE_PYTHON_PYGAME_MIXER),y)
PYTHON_PYGAME_OPT_DEPENDS += sdl_mixer sdl2_mixer
endif

PYTHON_PYGAME_DEPENDENCIES = sdl2 freetype sdl_compat host-python-cython $(PYTHON_PYGAME_OPT_DEPENDS)
HOST_PYTHON_PYGAME_DEPENDENCIES = host-python-cython
...
define PYTHON_PYGAME_CONFIGURE_CMDS
	...
	$(SED) 's~^FREETYPE = -lfreetype~FREETYPE = -I/usr/include/freetype2 -lfreetype~' $(@D)/Setup
	$(SED) 's/^pypm/#pypm/' $(@D)/Setup
	$(SED) 's/^display/#display/' $(@D)/Setup
	$(SED) 's/^event/#event/' $(@D)/Setup
